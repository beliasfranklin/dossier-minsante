...
<head>
    ...
    /* Classes de priorité pour un affichage adaptatif */
    .module-highlights.priority-high {
        border-left: 4px solid #667eea;
        background: linear-gradient(135deg, rgba(102, 126, 234, 0.02) 0%, rgba(255,255,255,0.98) 100%);
    }
    
    .module-highlights.priority-normal {
        border-left: 2px solid #94a3b8;
    }
    
    .module-highlights.priority-low {
        opacity: 0.9;
    ...
    /* Badges pour les actions rapides */
    .action-badge {
...
<body>
    <header class="app-header">
        ...
        <!-- Section des éléments importants du module (générée dynamiquement) -->
        <?php echo displayModuleHighlights(); ?>
    </header>
    ...
    function loadHighlightsData() {
        ...
        fetch('<?= BASE_URL ?>api/dashboard/highlights.php')
            ...
            .catch(error => {
                ...
                // Fallback avec des données par défaut selon le contexte
                updateHighlightsDisplay(getDefaultHighlightsData());
            });
    }
    
    function getDefaultHighlightsData() {
        // Données par défaut selon la page actuelle
        const currentPage = window.location.pathname.split('/').pop().split('.')[0];
        
        const defaults = {
            dashboard: { retard: 0, urgent: 0, nouveaux: 0, validation_rate: 0 },
            list: { visible_count: 0, selected_count: 0, filters_count: 0 },
            stats: { period_days: 30, general_trend: 0 },
            admin: { online_users: 0, system_status: 'OK', storage_percent: 0 }
        };
        
        return defaults[currentPage] || defaults.dashboard;
    }
    
    function updateHighlightsDisplay(data) {
        // Mise à jour intelligente selon les éléments présents dans la page
        const highlightCards = document.querySelectorAll('.highlight-card');
        
        highlightCards.forEach(card => {
            const valueElement = card.querySelector('.highlight-value');
            if (!valueElement) return;
            
            const elementId = valueElement.id;
            let newValue = '--';
            ...
            switch (elementId) {
                case 'retard':
                case 'retardCount':
                    newValue = data.retard || 0;
                    updateCardUrgency(card, newValue, [0, 3, 10]);
                    break;
                    
                case 'urgent':
                case 'urgentCount':
                    newValue = data.urgent || 0;
                    updateCardUrgency(card, newValue, [0, 5, 15]);
                    break;
                    
                case 'nouveaux':
                case 'newCount':
                    newValue = data.nouveaux || 0;
                    updateCardUrgency(card, newValue, [0, 10, 25]);
                    break;
                    
                case 'validation':
                case 'validationRate':
                    newValue = (data.validation_rate || 0) + '%';
                    break;
                    
                case 'visible_count':
                    newValue = document.querySelectorAll('.dossier-row:not(.hidden)').length || 0;
                    break;
                    
                case 'selected_count':
                    newValue = document.querySelectorAll('.dossier-checkbox:checked').length || 0;
                    break;
                    
                case 'filters_count':
                    newValue = countActiveFilters();
                    break;
                    
                case 'period_days':
                    newValue = data.period_days || 30;
                    break;
                    
                case 'general_trend':
                    newValue = formatTrendValue(data.general_trend || 0);
                    break;
                    
                case 'online_users':
                    newValue = data.online_users || 0;
                    break;
                    
                case 'system_status':
                    newValue = data.system_status || 'OK';
                    updateSystemStatus(card, newValue);
                    break;
                    
                case 'storage_percent':
                    newValue = (data.storage_percent || 0) + '%';
                    updateStorageStatus(card, data.storage_percent || 0);
                    break;
                    
                case 'notifications_count':
                    newValue = data.notifications_count || 0;
                    break;
                    
                default:
                    if (data[elementId] !== undefined) {
                        newValue = data[elementId];
                    }
                    break;
            }
            
            // Animation de mise à jour
            updateCounterWithAnimation(valueElement, newValue);
        });
        
        // Mettre à jour les actions rapides si nécessaire
        updateQuickActions(data);
    }
    
    function updateCardUrgency(card, value, thresholds) {
    ...
    function updateSystemStatus(card, status) {
        card.classList.remove('urgent', 'important', 'success', 'info');
    ...
    function updateStorageStatus(card, percent) {
        card.classList.remove('urgent', 'important', 'success', 'info');
    ...
    function countActiveFilters() {
        // Compter les filtres actifs sur la page
        let count = 0;
        
        // Recherche de champs de filtre communs
        ...
        // Vérifier l'URL pour les paramètres de filtre
        const urlParams = new URLSearchParams(window.location.search);
        const filterParams = ['filter', 'status', 'category', 'date_start', 'date_end'];
        filterParams.forEach(param => {
            if (urlParams.get(param)) count++;
        });
    ...
    function updateQuickActions(data) {
        // Mettre à jour les badges sur les actions rapides si nécessaire
        const quickActions = document.querySelectorAll('.quick-action-btn');
        
        quickActions.forEach(action => {
            const text = action.textContent.toLowerCase();
            
            if (text.includes('notification') && data.notifications_count > 0) {
                addBadgeToAction(action, data.notifications_count);
            }
            
            if (text.includes('sélection') && data.selected_count) {
                addBadgeToAction(action, data.selected_count);
            }
        });
    }
    
    function addBadgeToAction(actionElement, count) {
        // Retirer le badge existant s'il y en a un
        const existingBadge = actionElement.querySelector('.action-badge');
        if (existingBadge) {
        ...
        if (count > 0) {
            ...
            badge.style.cssText = `
                ...
                font-weight: 600;
            `;
            actionElement.appendChild(badge);
        }
    }
    ...
    function updateCounterWithAnimation(element, newValue) {
        ...
        // Animation simple pour les changements de valeur
