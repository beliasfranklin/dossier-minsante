 ...
 * Ce script doit être exécuté via CRON toutes l    /**
     * Traiter les délais et escalades
     */
    private function processDeadlines() {
        ...
        try {
            ...
            $startedAtExists = $this->pdo->query("SHOW COLUMNS FROM workflow_instances LIKE 'started_at'")->rowCount() > 0;
            $workflowDeadlinesExists = $this->pdo->query("SHOW TABLES LIKE 'workflow_deadlines'")->rowCount() > 0;
            
            if (!$startedAtExists || !$workflowDeadlinesExists) {
                $this->log("Tables ou colonnes workflow manquantes, saut du traitement des délais");
                return;
            }
            ...
            $stmt = $this->pdo->query("
                SELECT wi.*, 
                       ...
                       wd.escalate_to_role, 
                       wd.auto_approve,
                       TIMESTAMPDIFF(HOUR, COALESCE(wi.started_at, wi.created_at), NOW()) as hours_elapsed
                FROM workflow_instances wi
                LEFT JOIN workflow_deadlines wd ON wi.workflow_step_id = wd.workflow_step_id
                WHERE wi.status = 'active'
                AND COALESCE(wi.started_at, wi.created_at) IS NOT NULL
                AND TIMESTAMPDIFF(HOUR, COALESCE(wi.started_at, wi.created_at), NOW()) >= COALESCE(wd.deadline_hours, 72)
            ");
            
            $overdueInstances = $stmt->fetchAll();
            
        } catch (Exception $e) {
            $this->log("Erreur lors du traitement des délais: " . $e->getMessage());
            $overdueInstances = [];
