...
class AnalyticsManager {
    ...
    private function calculateTrends($days) {
        try {
            $stmt = $this->db->prepare("
                ...
                FROM dossiers
            ");
            // Paramètres: $days, $days*2, $days, $days, $days*2, $days (6 paramètres)
            $stmt->execute([$days, $days * 2, $days, $days, $days * 2, $days]);
            ...
            error_log("Erreur calculateTrends: " . $e->getMessage());
            return [
                'dossiers' => 0,
                'validations' => 0
            ];
    ...
    public function getTimeSeriesData($metric = 'created', $period = '30d', $groupBy = 'day') {
        try {
            ...
            error_log("Erreur getTimeSeriesData: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getPriorityDistribution() {
        try {
            ...
            error_log("Erreur getPriorityDistribution: " . $e->getMessage());
            return [
                ['priority' => 'urgent', 'count' => 0, 'percentage' => 0],
    ...
    public function getUserPerformance($limit = 10) {
        try {
            ...
            error_log("Erreur getUserPerformance: " . $e->getMessage());
    ...
    public function getServicePerformance() {
        try {
            ...
            error_log("Erreur getServicePerformance: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getDeadlineAnalysis() {
        try {
            ...
            error_log("Erreur getDeadlineAnalysis: " . $e->getMessage());
            return [
                'en_retard' => 0,
                'echeance_3j' => 0,
                'echeance_7j' => 0,
                'echeance_30j' => 0,
                'marge_moyenne_respectee' => 0
            ];
        }
    }
    ...
    public function getMonthlyPerformance($months = 12) {
        try {
            ...
            error_log("Erreur getMonthlyPerformance: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getRealtimeData() {
        try {
            ...
            error_log("Erreur getRealtimeData: " . $e->getMessage());
            return [
                'timestamp' => time(),
