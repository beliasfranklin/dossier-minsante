...
");
$stmt->execute([$dossier['status'], $_SESSION['user_role']]);
$transitions_autorisees = $stmt->fetchAll();

// Récupération des pièces jointes - Solution robuste
$attachments = [];
try {
    ...
    foreach ($possibleTables as $table) {
        try {
            $stmt = $pdo->prepare("SELECT * FROM {$table} WHERE dossier_id = ? ORDER BY uploaded_at DESC");
            $stmt->execute([$dossierId]);
    ...
    // Si aucune table n'existe, utiliser la fonction si disponible
    if (empty($attachments) && function_exists('getDossierAttachments')) {
        try {
            $attachments = getDossierAttachments($dossierId);
        } catch (Exception $e) {
            // Ignorer l'erreur, continuer avec tableau vide
        }
    }
    
} catch (Exception $e) {
    error_log("Erreur lors de la récupération des pièces jointes: " . $e->getMessage());
    $attachments = []; // Assurer un tableau vide en cas d'erreur
}
...
// Récupération des étapes de workflow
$workflowSteps = [];
try {
    // Essayer d'abord la table workflows
    $stmt = $pdo->prepare("
        SELECT w.*, 'etape' as etape, role_requis 
        FROM workflows w 
        WHERE w.type_dossier = ? 
        ORDER BY w.ordre ASC
    ");
    $stmt->execute([$dossier['category_id'] ?? 'general']);
    $workflowSteps = $stmt->fetchAll();
    
    // Si pas de workflow spécifique, créer un workflow basique à partir des transitions
    if (empty($workflowSteps)) {
        $stmt = $pdo->query("
            SELECT DISTINCT from_status as etape, role_requis 
            FROM status_transitions 
            WHERE actif = 1 
            ORDER BY role_requis ASC
        ");
        $workflowSteps = $stmt->fetchAll();
        
        // Si toujours vide, créer un workflow par défaut
        if (empty($workflowSteps)) {
            $workflowSteps = [
                ['etape' => 'brouillon', 'role_requis' => 4],
                ['etape' => 'en_cours', 'role_requis' => 3],
                ['etape' => 'en_attente', 'role_requis' => 2],
                ['etape' => 'valide', 'role_requis' => 1]
            ];
        }
    }
} catch (PDOException $e) {
    // En cas d'erreur, workflow par défaut
    $workflowSteps = [
        ['etape' => 'brouillon', 'role_requis' => 4],
        ['etape' => 'en_cours', 'role_requis' => 3],
        ['etape' => 'en_attente', 'role_requis' => 2],
    ...
    error_log("Erreur workflow: " . $e->getMessage());
}
                                ...
                                <div class="d-flex align-items-center justify-content-between">
                                    <div class="attachment-info">
                                        <div class="attachment-name font-weight-bold">
                                            ...
                                            $fileName = $attachment['original_name'] ?? $attachment['nom_fichier'] ?? $attachment['name'] ?? 'Fichier';
                                            echo htmlspecialchars(substr($fileName, 0, 30));
                                            echo strlen($fileName) > 30 ? '...' : '';
                                            ?>
                                        </div>
                                        <div class="attachment-meta text-muted small">
                                            <?php 
                                            // Gérer différentes structures pour la taille
                                            $fileSize = $attachment['file_size'] ?? $attachment['taille'] ?? $attachment['size'] ?? 0;
                                            echo formatFileSize($fileSize);
                                            ?> • 
                                            <?php 
                                            // Gérer différentes structures pour la date
                                            $uploadDate = $attachment['uploaded_at'] ?? $attachment['date_creation'] ?? $attachment['created_at'] ?? date('Y-m-d');
                                            echo date('d/m/Y', strtotime($uploadDate));
                                            ?>
                                        </div>
                                    </div>
                                    <div class="attachment-actions">
                                        <?php 
                                        // Gérer différentes structures pour le nom de fichier
                                        $downloadFileName = $attachment['file_name'] ?? $attachment['nom_stockage'] ?? $attachment['filename'] ?? $fileName;
                                        ?>
                                        <a href="<?= BASE_URL ?>uploads/<?= htmlspecialchars($downloadFileName) ?>" 
        ...
        <div class="sidebar">
            ...
            <div class="sidebar-card" style="background:#fff;border-radius:16px;padding:20px;box-shadow:0 2px 12px rgba(41,128,185,0.08);margin-bottom:20px;">
                ...
                <div class="actions-list" style="display:flex;flex-direction:column;gap:8px;">
                    <?php if (canEditDossier($_SESSION['user_id'], $_SESSION['user_role'], $dossier['responsable_id'])): ?>
                    ...
                    <?php if (!empty($transitions_autorisees) && canEditDossier($_SESSION['user_id'], $_SESSION['user_role'], $dossier['responsable_id'])): ?>
                        <hr style="margin: 16px 0; border: none; height: 1px; background: #e1e8ed;">
                        <div class="status-actions">
                            <h4 style="color:#2980b9;font-size:0.9em;margin:0 0 12px 0;text-transform:uppercase;letter-spacing:0.5px;">
                                <i class="fas fa-exchange-alt"></i> Changer le statut
                            </h4>
                            <?php foreach ($transitions_autorisees as $nouveau_statut => $label): ?>
                                <button type="button" 
                                        class="btn btn-outline-info btn-sm change-status mb-1" 
                                        data-status="<?= htmlspecialchars($nouveau_statut) ?>"
