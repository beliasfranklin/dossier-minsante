...
<div class="stats-container">
                    ...
                    <tr>
                        ...
                        <td>
                            <span style="font-weight: 600; color: var(--text-secondary);">
                                <?= $advanced_stats['total_dossiers'] > 0 ? round(($service['count'] / $advanced_stats['total_dossiers']) * 100, 1) : 0 ?>%
                            </span>
                        </td>
                        <td>
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span style="font-weight: 600; color: var(--text-secondary);">
                                <?= $advanced_stats['total_dossiers'] > 0 ? round(($type['count'] / $advanced_stats['total_dossiers']) * 100, 1) : 0 ?>%
                            </span>
                        </td>
                        <td>
    ...
    <!-- Utilisateurs les plus actifs -->
    <?php if (!empty($stats_users_active)): ?>
    <div class="data-section">
        <div class="section-header">
            <div class="section-icon">
        ...
        <div class="chart-container">
            <table class="data-table">
                ...
                <tbody>
                    <?php foreach ($stats_users_active as $user): ?>
                    <tr>
                        <td>
                            <div style="display: flex; align-items: center; gap: 12px;">
                                <div style="width: 40px; height: 40px; border-radius: 50%; background: linear-gradient(135deg, var(--stats-primary), var(--stats-secondary)); display: flex; align-items: center; justify-content: center; color: white; font-weight: 700; font-size: 0.9rem;">
                                    <?= strtoupper(substr($user['nom'] ?: 'U', 0, 1) . substr($user['prenom'] ?: 'N', 0, 1)) ?>
                                </div>
                                <div>
                                    <div style="font-weight: 600; color: var(--text-primary);">
                                        <?= htmlspecialchars(($user['nom'] ?: 'Nom') . ' ' . ($user['prenom'] ?: 'Prénom')) ?>
                                    </div>
                                    <div style="font-size: 0.85rem; color: var(--text-secondary);">
                                        <?= htmlspecialchars($user['email'] ?: 'Email non disponible') ?>
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td>
                            <span class="role-badge role-<?= strtolower($user['role'] ?: 'consultant') ?>">
                                <i class="fas fa-<?= ($user['role'] ?? '') === 'admin' ? 'crown' : (($user['role'] ?? '') === 'gestionnaire' ? 'user-tie' : 'eye') ?>"></i>
                                <?= ucfirst($user['role'] ?: 'Consultant') ?>
                            </span>
                        </td>
                        <td>
                        ...
                        <td style="color: var(--text-secondary);">
                            <?= $user['derniere_activite'] ? date('d/m/Y H:i', strtotime($user['derniere_activite'])) : 'Non disponible' ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
    ...
    <div class="data-section">
        ...
        <div class="chart-container">
            <div class="timeline-chart">
                <?php 
                $max_count = !empty($stats_evolution) ? max(array_column($stats_evolution, 'count')) : 1;
                foreach ($stats_evolution as $month): 
                ?>
                    <div class="timeline-bar" 
                         style="height: <?= ($month['count'] / max($max_count, 1)) * 160 ?>px;"
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span style="font-weight: 600; color: var(--text-secondary);">
                                <?php 
                                $total_users = array_sum(array_column($stats['users'], 'count'));
                                echo $total_users > 0 ? round(($row['count'] / $total_users) * 100, 1) : 0;
                                ?>%
                            </span>
                        </td>
    ...
    <?php if (empty($stats_dossiers) && empty($stats_services) && empty($stats_types) && empty($stats_users_active)): ?>
    <div class="data-section">
