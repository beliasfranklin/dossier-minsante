...
if ($_POST) {
    try {
        ...
        // Ne traiter que les champs qui sont présents dans $_POST
        if (isset($_POST['theme'])) {
            $preferences['theme'] = $_POST['theme'];
        }
        if (isset($_POST['dashboard_layout'])) {
            $preferences['dashboard_layout'] = $_POST['dashboard_layout'];
        }
        ...
        if (isset($_POST['items_per_page'])) {
            $preferences['items_per_page'] = (int)$_POST['items_per_page'];
        }
        
        // Traitement spécial pour les checkboxes (notifications)
        if (array_key_exists('notifications_email', $_POST)) {
            $preferences['notifications_email'] = isset($_POST['notifications_email']) ? 1 : 0;
        }
        if (array_key_exists('notifications_browser', $_POST)) {
            $preferences['notifications_browser'] = isset($_POST['notifications_browser']) ? 1 : 0;
        }

        // Sauvegarder seulement les préférences qui ont été soumises
        ...
        $success = "Préférences mises à jour avec succès ! (" . count($preferences) . " paramètre(s) modifié(s))";
        
        // Debug (à supprimer en production)
        if (isset($_GET['debug'])) {
            $success .= "<br><small>Champs traités : " . implode(', ', array_keys($preferences)) . "</small>";
        }
        
        // Redirection pour éviter la resoumission et forcer le rechargement
        if (!isset($_GET['debug'])) {
            $_SESSION['preferences_success'] = $success;
...
if (isset($_SESSION['preferences_success'])) {
    $success = $_SESSION['preferences_success'];
...
try {
    ...
    if (isset($_GET['debug'])) {
        ...
        echo "<strong>DEBUG - Préférences récupérées :</strong><br>";
        echo "User ID: $userId<br>";
        echo "Nombre de préférences: " . count($preferences) . "<br>";
        if (!empty($preferences)) {
            foreach ($preferences as $k => $v) {
                echo "- $k = '$v'<br>";
            }
        } else {
            echo "<span style='color: red;'>AUCUNE PRÉFÉRENCE TROUVÉE !</span><br>";
        }
        echo "</div>";
    }
    ...
    // Debug de l'erreur
    if (isset($_GET['debug'])) {
        echo "<div style='background: #ffe6e6; padding: 10px; border: 1px solid #cc0000; margin: 10px 0;'>";
        echo "<strong>DEBUG - Erreur DB :</strong> " . $e->getMessage();
        echo "</div>";
    }
...
<div class="container">
    ...
    <!-- Section de test pour développement -->
    <div class="preference-card" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.1); overflow: hidden; margin-top: 2rem;">
        <div class="card-header" style="background: rgba(255,255,255,0.1); padding: 1.5rem; border-bottom: 1px solid rgba(255,255,255,0.2);">
            <h3 style="margin: 0; color: white; display: flex; align-items: center; gap: 12px;">
                <i class="fas fa-flask" style="color: #ffd700;"></i>
                Tests de Fonctionnement
            </h3>
            <p style="margin: 0.5rem 0 0 0; color: rgba(255,255,255,0.8);">Vérifiez que le système de préférences fonctionne correctement</p>
        </div>
        
        <div style="padding: 1.5rem;">
            <div class="test-buttons" style="display: flex; gap: 1rem; flex-wrap: wrap; margin-bottom: 1rem;">
                <button type="button" class="btn" onclick="testThemeChange()" style="background: rgba(255,255,255,0.2); color: white; border: 1px solid rgba(255,255,255,0.3); padding: 8px 16px; border-radius: 6px; cursor: pointer; transition: all 0.2s;">
                    <i class="fas fa-palette"></i> Test Thème
                </button>
                <button type="button" class="btn" onclick="testLayoutChange()" style="background: rgba(255,255,255,0.2); color: white; border: 1px solid rgba(255,255,255,0.3); padding: 8px 16px; border-radius: 6px; cursor: pointer; transition: all 0.2s;">
                    <i class="fas fa-th"></i> Test Layout
                </button>
                <button type="button" class="btn" onclick="window.open('<?= BASE_URL ?>test/test_preferences_system.php', '_blank')" style="background: rgba(255,255,255,0.2); color: white; border: 1px solid rgba(255,255,255,0.3); padding: 8px 16px; border-radius: 6px; cursor: pointer; transition: all 0.2s;">
                    <i class="fas fa-external-link-alt"></i> Tests Complets
                </button>
            </div>
            
            <div id="test-output" style="display: none; background: rgba(255,255,255,0.1); padding: 1rem; border-radius: 8px; margin-top: 1rem;">
                <strong style="color: white;">Résultats du test :</strong>
                <div id="test-results" style="color: rgba(255,255,255,0.9); margin-top: 0.5rem;"></div>
            </div>
        </div>
    </div>
...
<script>
// Tests JavaScript pour les préférences
function testThemeChange() {
    ...
    if (window.preferencesManager) {
        const currentTheme = window.preferencesManager.getTheme();
        ...
        setTimeout(() => {
            results.innerHTML = `
                ✅ Thème changé de "${currentTheme}" vers "${newTheme}"<br>
                ✅ PreferencesManager fonctionnel<br>
                ✅ Animation de transition appliquée<br>
                🎨 Nouvel état: ${window.preferencesManager.getTheme()}
            `;
        }, 1000);
    } else {
        results.innerHTML = '❌ PreferencesManager non disponible - Vérifiez que le script JavaScript est chargé';
    }
}
...
function testLayoutChange() {
    ...
    output.style.display = 'block';
    results.innerHTML = '🔄 Test du layout en cours...';
    
    const container = document.querySelector('.preferences-container');
    if (container) {
        const currentLayout = container.getAttribute('data-layout') || 'grid';
        ...
        setTimeout(() => {
            results.innerHTML = `
                ✅ Layout changé de "${currentLayout}" vers "${newLayout}"<br>
                ✅ Animation de transition appliquée<br>
                ✅ Container mis à jour<br>
                📊 Nouvel état: ${newLayout}
            `;
        }, 500);
    } else {
        results.innerHTML = '❌ Container .preferences-container non trouvé';
    }
}

// Test de vérification au chargement
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Page settings chargée avec système de tests');
    
    // Test automatique discret
    if (window.preferencesManager) {
        console.log('✅ PreferencesManager détecté et fonctionnel');
        console.log('🎨 Thème actuel:', window.preferencesManager.getTheme());
        console.log('🌍 Langue actuelle:', window.preferencesManager.getLanguage());
    } else {
        console.log('⚠️ PreferencesManager non détecté - vérification en cours...');
        
        // Réessayer après 1 seconde
        setTimeout(() => {
            if (window.preferencesManager) {
                console.log('✅ PreferencesManager détecté après délai');
            } else {
                console.log('❌ PreferencesManager définitivement non disponible');
            }
        }, 1000);
    }
});
