...
function predictDelays() {
    ...
    // Analyser les dossiers en cours
    $stmt = $pdo->query("
        SELECT 
            d.*,
            wi.started_at,
            wd.deadline_hours,
            TIMESTAMPDIFF(HOUR, wi.started_at, NOW()) as elapsed_hours,
            (wd.deadline_hours - TIMESTAMPDIFF(HOUR, wi.started_at, NOW())) as remaining_hours
        FROM dossiers d
        JOIN workflow_instances wi ON d.id = wi.dossier_id
