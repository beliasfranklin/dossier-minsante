...
// Goulots d'étranglement (workflow)
$goulots = fetchAll("
    SELECT 
        w.etape,
        w.type_dossier,
        COUNT(wi.id) as nombre_instances,
        AVG(TIMESTAMPDIFF(HOUR, wi.started_at, COALESCE(wi.approved_at, wi.rejected_at, NOW()))) as duree_moyenne_heures,
        COUNT(CASE WHEN wi.status = 'active' THEN 1 END) as en_attente,
    ...
    GROUP BY w.id, w.etape, w.type_dossier
